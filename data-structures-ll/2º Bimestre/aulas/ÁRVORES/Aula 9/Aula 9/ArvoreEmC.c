...
main() {
	Arv* a1= cria(1,inicializa(),inicializa()); /* sub-árvore com '2'*/ //filho de 2, ñ tem folha adiciona NULL a esquerda e direita.
	Arv* a2= cria(2,inicializa(), a1); /* sub-árvore com '1'*/ //a1 indica que já está ocupado
	Arv* a3= cria(3,inicializa(),inicializa()); /* sub-árvore com 'e'*/ // folha, n tem filho, inicializa do dois lados
	Arv* a4= cria(4,inicializa(),inicializa()); /* sub-árvore com 'f'*/ // verifica se é NULL, se for insere, senão indica que já está ocupado
	Arv* a5= cria(5,a3,a4); /* sub-árvore com 'c'*/ // a5 é pai de a3(3) e a4(4)
	Arv* a = cria(6,a2,a5 ); /* árvore com raiz 'a'*/ // RAIZ aponta para B e C
	imprime(a);
	puts("");
	imprime_alt(a);
	...
	// int busca (Arv* a, char c);
	printf("\nint busca (Arv* a, char c) %d", busca (a, 'a')); // existe a na árvore?
	printf("\nint busca (Arv* a, char c) %d", busca (a, 'x'));
	libera(a); // vai no ultimo node e apaga e vai voltando
	int soma = conta(a);
	printf("\nSoma %d", soma);
}
