...
int VisitaBfs(link a, int Dist[], char cor[], int antecessor[], vertex v)
{
	link Aux;
	int peso;
	int item;
...
int BuscaEmLargura(Graph G)
{
	link a;
	int Dist[10000];
	char cor[10000];
	int antecessor[10000];
	vertex v = 1;
	char branco[128] = "branco";
	for(a = G->adj[v]; a != NULL; a = a->next)
	{
		v++;
		strcpy(cor, "branco");
	...
	//printf("%s\n", cor);
		...
		{
			printf("hjdjdj");
			VisitaBfs(a, Dist, cor, antecessor, v);
		}
	}
}
